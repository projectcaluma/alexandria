...
from ..models import TagSynonymGroup, Document, Tag
...
@pytest.mark.parametrize(
    "value,amount",
    [
        ("foo_tag2", 1),  # should match as a synonym of foo_tag
        ("foo_tag", 1),
        ("_t", 2),
        ("foo_title", 1),
        ("filename.pdf", 1),
...
def test_tag_synonym_filter(
    ...
    for i in range(1, ngroups + 1):
        group = f"synonyms{i}"
        groups[group] = None
        locals()[group] = [f"group{i}_syn{n}" for n in range(1, groupsize + 1)]
    ...
    for key, value in groups.items():
        groups[key] = create_tagged_docs(locals()[key])
    ...
    for tags in tag_sets_base:
        ...
        assert resp.status_code == HTTP_200_OK
        result = resp.json()
        assert [str(doc.id) for doc in tag_group_synonyms1_tagged_documents] == [
            obj["id"] for obj in result["data"]
        ]
    ...
    for tag in ungrouped:
        ...
        assert resp.status_code == HTTP_200_OK
        result = resp.json()
        assert [str(doc.id) for doc in Tag.objects.get(slug=tag).documents.all()] == [
            obj["id"] for obj in result["data"]
        ]
