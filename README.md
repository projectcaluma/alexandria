...
- `MANABI_TOKEN_REFRESH_TIMEOUT`: Defines how often tokens are refreshed in seconds during an active WebDAV editing session. The default is `3600` (60 minutes). This is set to a relatively long timeout because since MS Word 2509, the server-sent lock timeout is ignored and re-locking is done after ~40 minutes, see [Microsoft Docs](https://learn.microsoft.com/en-us/answers/questions/5629856/webdav-lock-issues-with-ms-office-v2509-monthly-cu).
- `MANABI_TOKEN_ACTIVATE_TIMEOUT`: Maximum time in seconds from when a token is generated by Alexandria until it must be activated/used for the first time. This timeout should be kept short to minimize security risk. After this initial activation, tokens are kept alive via `MANABI_TOKEN_REFRESH_TIMEOUT`. Default is `600` (10 minutes).
- `MANABI_DEBUG`: Enables verbose logging for WebDAV.
